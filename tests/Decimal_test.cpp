...
TEST(DecimalTest, Constructor) {
    ...
    Decimal decimal_1 = "0";
    EXPECT_STREQ(decimal_1.get_value().c_str(), "0.0");
    Decimal decimal_2 = "9876543210";
    EXPECT_STREQ(decimal_2.get_value().c_str(), "9876543210.0");
    ...
    EXPECT_STREQ(decimal_5.get_value().c_str(), "123.0");
    Decimal decimal_6 = "-00123.0000";
    EXPECT_STREQ(decimal_6.get_value().c_str(), "-123.0");
    Decimal decimal_7 = "1/5";
    EXPECT_STREQ(decimal_7.get_value().c_str(), "0.2");
    Decimal decimal_8 = "1/104729";
    EXPECT_STREQ(decimal_8.get_value().c_str(), "0.0000095484536279349559338865070801783651137698249768449999522577318603252203305674645991081744311508");
    Decimal decimal_9 = "0";
    Decimal decimal_10 = decimal_9;
    decimal_9 = "1";
    EXPECT_STREQ(decimal_9.get_value().c_str(), "1.0");
    EXPECT_STREQ(decimal_10.get_value().c_str(), "0.0");
}
...
TEST(DecimalTest, OutputOperator) {
    ...
    input_ss2 << "1234.0000";
    input_ss2 >> decimal;
    EXPECT_STREQ(decimal.get_value().c_str(), "1234.0");
...
TEST(DecimalTest, AssignmentOperator) {
    ...
    Decimal decimal_2;
    decimal_2 = decimal;
    EXPECT_STREQ(decimal.get_value().c_str(), "0.0");
    EXPECT_STREQ(decimal_2.get_value().c_str(), "0.0");
    decimal_2 = "1";
    EXPECT_STREQ(decimal.get_value().c_str(), "0.0");
    EXPECT_STREQ(decimal_2.get_value().c_str(), "1.0");
    decimal = "-1";
    EXPECT_STREQ(decimal.get_value().c_str(), "-1.0");
    EXPECT_STREQ(decimal_2.get_value().c_str(), "1.0");
}
...
TEST(DecimalTest, MultiplicationOperator) {
    ...
    Decimal g = "-1", h = "-1";
    EXPECT_STREQ((g * h).get_value().c_str(), "1.0");
}
...
TEST(DecimalTest, DivisionOperator) {
    Decimal a = "64", b = "8";
    EXPECT_STREQ((a / b).get_value().c_str(), "8.0");
    Decimal c = "20", d = "0.2";
    EXPECT_STREQ((c / d).get_value().c_str(), "100.0");
    Decimal e = "1/3", f = "2/3";
    EXPECT_STREQ((e / f).get_value().c_str(), "0.5");
}
